...
import { HiOutlineServerStack, HiOutlineBookOpen } from 'react-icons/hi2';
...
const Sidebar = ({ servers, sidebarOpen, setSidebarOpen }: SidebarProps) => {
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
                <NavLink to="/">
                    <img src={Logo} alt="SSLPanel" className='h-7' />
                </NavLink>
                        ...
                        <ul className="mb-6 flex flex-col gap-1.5">
                            {/* <!-- Menu Item Servers --> */}
                            {
                                !servers.length ? (
                                    ...
                                    <SidebarLinkGroup activeCondition={pathname === '/servers' || pathname.includes('servers')}>
                                        {(handleClick, open) => {
                                            return (
                                                <Fragment>
                                                    <NavLink
                            ...
                            <SidebarLinkGroup activeCondition={pathname === '/documentation' || pathname.includes('documentation')}>
                                {(handleClick, open) => {
                                    return (
                                        <Fragment>
                                            <NavLink
                                                to="#"
                                                className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${(pathname === '/documentation' || pathname.includes('documentation')) && 'bg-graydark dark:bg-meta-4'}`}
                                                onClick={event => {
                                                    event.preventDefault();
                                                    sidebarExpanded ? handleClick() : setSidebarExpanded(true);
                                                }}
